    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('mahasiswa.update', $mahasiswa->npm) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                    @method('patch')
                    @csrf
                    ...
                    <div class="max-w-xl">
                        ...
                        <x-text-input id="npm" type="text" name="npm" class="mt-1 block w-full" value="{{ old('npm', $mahasiswa->npm)}}" required/>
                        <x-input-error class="mt-2" :messages="$errors->get('npm')" />
                    </div>

                    <div class="max-w-xl">
                        <x-input-label for="nama" value="NAMA"/>
                        <x-text-input id="nama" type="text" name="nama" class="mt-1 block w-full" value="{{ old('nama', $mahasiswa->nama)}}" required/>
                    ...
                    <div class="max-w-xl">
                        ...
                        <select name="nidn" id="nidn" required>
                            <option value=""></option>
                            @foreach ($dosen as $mhs)
                                    <option value="{{ $mhs->nidn }}"> {{ $mhs->nama }} </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        href="{{ route('mahasiswa.index') }}">Cancel</a>
